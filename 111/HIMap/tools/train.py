...
def get_model_parameters_number_lidar(model):
    params_num = 0
    for k, p in model.named_parameters():
        if 'img_backbone' in k or 'img_neck' in k or 'upsample' in k:
            continue
        if p.requires_grad:
            params_num += p.numel()
    return params_num
...
def parse_args():
    ...
    parser.add_argument('--resume_from_latest', action='store_true',
                        help='find the latest checkpoint file to resume from')
    parser.add_argument(
        '--no-validate',
        action='store_true',
...
def main():
    ...
    #     if not args.resume_from_latest:
    #         cfg.resume_from = args.resume_from
    #     else:
    #         # find the latest file.
    #         print("start find the latest checkpoint to resume ...............")
    #         curr_resume_epoch = int((args.resume_from).split('_')[-1].split('.')[0])
    #         for f_i in range(110, curr_resume_epoch, -2):
    #             find_resume_file = '_'.join((args.resume_from).split('_')[:-1]) + '_' + str(f_i) + '.pth'
    #             if osp.isfile(find_resume_file):
    #                 cfg.resume_from = find_resume_file
    #                 print("find the latest checkpoint {} to resume ...............".format(cfg.resume_from))
    #                 break
    
    e110_pth = osp.join(cfg.work_dir, 'epoch_110.pth')
    la_pth = osp.join(cfg.work_dir, 'latest.pth')
    if osp.isfile(la_pth) and not osp.isfile(e110_pth):
        cfg.resume_from = la_pth
    ...
    print("Model Params : {}".format(params_to_string(get_model_parameters_number_lidar(model))))
    print("========================")

    for k, _ in model.named_parameters():
        print(k)
    print("print parameters finish.............................................")
