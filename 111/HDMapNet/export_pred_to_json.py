...
def _import_torch():
    """Import torch before mmcv; retry on intermittent `_C` NameError."""
    try:
...
def export_to_json(model, val_loader, angle_class, args):
    branch_mode = getattr(args, "branch_mode", None)
    if branch_mode == "fusion":
        branch_mode = None
    submission = {
        "meta": {
            "use_camera": branch_mode != "lidar_only",
            "use_lidar": (
                branch_mode == "lidar_only"
                or (branch_mode is None and "fusion" in args.model.lower())
                or "lidar" in args.model.lower()
            ),
            "use_radar": False,
            "use_external": False,
            "vector": True,
    ...
    with torch.no_grad():
        for batchi, (imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, segmentation_gt, instance_gt, direction_gt) in enumerate(tqdm.tqdm(val_loader)):
            segmentation, embedding, direction = model(
                imgs.cuda(),
                trans.cuda(),
                rots.cuda(),
                intrins.cuda(),
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Use camera_only / lidar_only when evaluating single-branch probes.",
    )
