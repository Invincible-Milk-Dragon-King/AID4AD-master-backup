...
def get_model(method, data_conf, instance_seg=True, embedded_dim=16, direction_pred=True, angle_class=36):
    if method == 'lift_splat':
        ...
        # Protocol-1 Full-L: same lidar branch as fusion (pp + lidar_bevencode),
        # trained/evaluated with branch_mode=lidar_only (not legacy PointPillar).
        model = HDMapNet(
            data_conf,
            instance_seg=instance_seg,
            embedded_dim=embedded_dim,
            direction_pred=direction_pred,
            direction_dim=angle_class,
            lidar=True,
        )
