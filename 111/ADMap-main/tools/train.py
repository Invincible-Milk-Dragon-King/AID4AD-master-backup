...
def apply_decoder_only_freeze(model, train_keywords, logger, freeze_keywords=None):
    ...
    for name, param in model.named_parameters():
        ...
        # LSS registers dx/bx/nx as Parameters(requires_grad=False). Forcing
        # them trainable lets Adam corrupt the BEV grid after step 1 → empty
        # bev_pool / CUDA illegal memory access on the next iter.
