...
class HDMapNetDataset(Dataset):
    def __init__(self, version, dataroot, prior_map_root, data_conf, is_train, satellite_map_size, is_newsplit=False):
        ...
        self.is_newsplit = is_newsplit
        self.data_conf = data_conf
        self.patch_size = (patch_h, patch_w)
        self.canvas_size = (canvas_h, canvas_w)
        self.nusc = NuScenes(version=version, dataroot=dataroot, verbose=False)
        self.prior_map = PriorMap(prior_map_root, img_size=satellite_map_size)
    ...
    def get_scenes(self, version, is_train):
        ...
        split = {
            ...
            'v1.0-test': {False: 'test'}
        }[version][is_train]

        if self.is_newsplit:
            print("______________ Using new splits ______________")
            from .new_splits import TRAIN_SCENES, VAL_SCENES
            return TRAIN_SCENES if is_train else VAL_SCENES
        else:
    ...
    def get_ego_pose(self, rec):
        ...
        prior_map = self.prior_map.get_prior_map(sample_data_record['ego_pose_token'])
        pos_rotation = Quaternion(ego_pose['rotation'])
        yaw_pitch_roll = pos_rotation.yaw_pitch_roll
        return torch.tensor(car_trans), torch.tensor(yaw_pitch_roll), prior_map
    ...
    def __getitem__(self, idx):
        ...
        car_trans, yaw_pitch_roll, prior_map = self.get_ego_pose(rec)
        vectors = self.get_vectors(rec)

        return imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, vectors, prior_map


class HDMapNetSemanticDataset(HDMapNetDataset):
    def __init__(self, version, dataroot, prior_map_root, data_conf, is_train, satellite_map_size, is_newsplit=False):
        super(HDMapNetSemanticDataset, self).__init__(version, dataroot, prior_map_root, data_conf, is_train, satellite_map_size, is_newsplit=is_newsplit)
    ...
    def __getitem__(self, idx):
        ...
        return imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, semantic_masks, instance_masks, direction_masks, prior_map


def semantic_dataset(version, dataroot, prior_map_root, data_conf, bsz, nworkers, multi_gpu,satellite_map_size, is_newsplit=False):
    train_dataset = HDMapNetSemanticDataset(version, dataroot, prior_map_root, data_conf, is_train=True, satellite_map_size=satellite_map_size, is_newsplit=is_newsplit)
    val_dataset = HDMapNetSemanticDataset(version, dataroot, prior_map_root, data_conf, is_train=False, satellite_map_size=satellite_map_size, is_newsplit=is_newsplit)
...
def semantic_dataset_dist(version, dataroot, prior_map_root, data_conf, bsz, nworkers, multi_gpu,satellite_map_size, is_newsplit=False):
    train_dataset = HDMapNetSemanticDataset(version, dataroot, prior_map_root, data_conf, is_train=True, satellite_map_size=satellite_map_size, is_newsplit=is_newsplit)
    val_dataset = HDMapNetSemanticDataset(version, dataroot, prior_map_root, data_conf, is_train=False,satellite_map_size=satellite_map_size, is_newsplit=is_newsplit)
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=bsz, shuffle=False, num_workers=nworkers,
...
def semantic_dataset_test(version, dataroot, prior_map_root, data_conf, bsz, nworkers, is_newsplit=False):
    test_dataset = HDMapNetSemanticDataset(version, dataroot, prior_map_root, data_conf, is_train=False, satellite_map_size=(400, 200), is_newsplit=is_newsplit)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=bsz, shuffle=False, num_workers=nworkers)
    return test_loader
...
if __name__ == '__main__':
    ...
    dataset = HDMapNetSemanticDataset(version='v1.0-mini', dataroot='/media/wjgao/Document/data/set/v1.0-mini', prior_map_root='/home/wjgao/Documents/PriorMapNet/prior_map_trainval', data_conf=data_conf, is_train=False)
    for idx in range(1):
    # for idx in range(dataset.__len__()):
        imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, semantic_masks, instance_masks, direction_mask, prior_map = dataset.__getitem__(idx)
