...
LOGDIR="${LOGDIR:-./branch_runs/camera_decoder_probe_fusion_distilled_newsplit}"
CHECKPOINT="${CHECKPOINT:-./branch_runs/fusion_distilled_camera_newsplit/model_last.pt}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" python -m torch.distributed.launch \
  ...
  --experiment_name camera_decoder_probe_fusion_distilled_newsplit \
  --prior_map_root ../AID4AD_ego_referenced \
  --train_decoder_only \
  --probe_feature_source fusion_distilled_camera \
