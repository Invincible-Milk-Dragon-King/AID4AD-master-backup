...
LOGDIR="${LOGDIR:-./branch_runs/sat_decoder_probe_fusion_base}"
CHECKPOINT="${CHECKPOINT:-./branch_runs/fusion_base/model_last.pt}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" python -m torch.distributed.launch \
  ...
  --experiment_name sat_decoder_probe_fusion_base \
  --prior_map_root ../AID4AD_ego_referenced \
  --train_decoder_only \
  --probe_feature_source fusion_base_sat \
