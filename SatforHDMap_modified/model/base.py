...
class CamEncode(nn.Module):
    def __init__(self, C):
        ...
        checkpoint = torch.load('model/adv-efficientnet-b0-b64d5a18.pth', map_location=torch.device('cpu'))
        self.trunk.load_state_dict(checkpoint)
...
class PriorMapEncoder(nn.Module):
    def __init__(self, c):
        super(PriorMapEncoder, self).__init__()
        self.inC = 3
        self.outC = c
        self.encoder = ResNetUNet(c)
