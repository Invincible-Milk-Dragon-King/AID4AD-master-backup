...
class HDMapNetLegacy(nn.Module):
    """Original HDMapNet camera/lidar topology adapted to the Satfor data interface."""

    def __init__(self, data_conf, args, instance_seg=True, embedded_dim=16, direction_pred=True, direction_dim=36, lidar=False):
        super(HDMapNetLegacy, self).__init__()
        self.camC = 64
        self.downsample = 16
    ...
    def forward(
        ...
        semantic, embedding, direction = self.bevencode(topdown)
        if return_branch_features:
            return {
                'semantic': semantic,
                'embedding': embedding,
                'direction': direction,
                'branch_mode': branch_mode or 'legacy',
                'camera_branch_feature': topdown,
                'satellite_branch_feature': None,
                'fusion_feature': topdown,
            }
