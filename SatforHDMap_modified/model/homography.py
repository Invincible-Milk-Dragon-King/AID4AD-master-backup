...
class IPM(nn.Module):
    def __init__(self, args, xbound, ybound, N, C, z_roll_pitch=False, visual=False, extrinsic=False, cuda=True):
        super(IPM, self).__init__()
        self.device_id = args.local_rank
        self.visual = visual
    ...
    def forward(self, images, Ks, RTs, translation, yaw_roll_pitch, post_RTs=None):
        ...
        zs = torch.tensor([0.]).to(self.device_id)
        planes = planes.cpu().to(self.device_id)
