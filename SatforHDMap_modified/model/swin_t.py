...
class Fusion_SwinTrans(nn.Module):
    def __init__(self,
                 bev_channels, prior_channels, hidden_c, align_fusion,
                 img_size=(400, 800), patch_size=(10, 10),
                 decoder_layers=2, dropout=0.1,
                 num_heads=8, window_size=5, mlp_ratio=4, qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm
                 ):
        super(Fusion_SwinTrans, self).__init__()
        ...
        self.patch_embedding = PatchEmbed(bev_in_channels=self.bev_channels, prior_in_channels=self.prior_channels, out_channels=self.hidden_c, img_size=img_size)
        self.decoder_layers = decoder_layers
        self.decoder = nn.ModuleList([
            SwinTransformerBlockForCrossAttention(dim=hidden_c, input_resolution=self.grid_size,
...
class PatchEmbed(nn.Module):
    def __init__(self, bev_in_channels, prior_in_channels, out_channels, img_size=(400,800), patch_size=(10, 10), dropout_rate=0.1):
...
class CrossWindowAttention(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C) for query
            y: input features with shape of (num_windows*B, N, C) for key, value
...
class SwinTransformerBlockForCrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.cross_attn(x_windows, y_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
