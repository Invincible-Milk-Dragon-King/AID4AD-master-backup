...
class NuscDataset(BaseMapDataset):
    ...
    def __init__(self, data_root, aerial_data_root=None, **kwargs):
        super().__init__(**kwargs)
        self.map_extractor = NuscMapExtractor(data_root, self.roi_size)
        self.renderer = Renderer(self.cat2id, self.roi_size, 'nusc')

        self.aerial_data_root = aerial_data_root
        if self.aerial_data_root != None:
            self.sample_token2aerial_fpath = {}
    ...
    def get_sample(self, idx):
        ...
        input_dict = {
            ...
            'aerial_image_path': self.sample_token2aerial_fpath[sample['token']] if self.aerial_data_root != None else None,
