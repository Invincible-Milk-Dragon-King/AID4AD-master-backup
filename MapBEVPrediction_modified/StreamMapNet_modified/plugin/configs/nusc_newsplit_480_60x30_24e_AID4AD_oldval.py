        ...
        assigner=dict(
            type='HungarianLinesAssigner',
                cost=dict(
                    ...
                    reg_cost=dict(type='LinesL1Cost', weight=50, beta=0.01, permute=permute),
                    ),
                ),
        ),
    streaming_cfg=dict(
...
train_pipeline = [
    ...
    dict(type='Collect3D', keys=['img', 'vectors', 'aerial_img'], meta_keys=(
...
test_pipeline = [
    ...
    dict(type='PadMultiViewImages', size_divisor=32),
    dict(type='FormatBundleMap'),
    dict(type='Collect3D', keys=['img', 'aerial_img'], meta_keys=(
        'token', 'ego2img', 'sample_idx', 'ego2global_translation',
        'ego2global_rotation', 'img_shape', 'scene_name'))
]

data_root = '../../nuScenes'
annot_dir = data_root + '/nuscenes_map_infos_'
aerial_image_dir = '../../AID4AD_ego_referenced'
...
eval_config = dict(
    type='NuscDataset',
    data_root=data_root,
    aerial_data_root=aerial_image_dir,
    ann_file=annot_dir + 'val.pkl',
    meta=meta,
    roi_size=roi_size,
    cat2id=cat2id,
    pipeline=[
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'aid_encoder': dict(lr_mult=0.1, decay_mult=1.0),
            'fuser': dict(lr_mult=0.5, decay_mult=1.0),
        }),
    weight_decay=1e-2)
