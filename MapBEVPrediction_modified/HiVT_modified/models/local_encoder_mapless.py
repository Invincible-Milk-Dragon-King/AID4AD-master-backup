...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.al_encoder = ALEncoder(node_dim=2, #2 for w/o uncertainty, 4 for w/ uncertainty
                                    edge_dim=2, #2 for w/o uncertainty, 4 for w/ uncertainty
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        #out = self.al_encoder(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr, rotate_mat=data['rotate_mat'])
        return out
...
class ALEncoder(MessagePassing):
    ...
    def message(self,
            ...
            if rotate_mat is None:
                ...
                # Without Uncertainty
                x_j = self.lane_embed([torch.bmm(x_j[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2),           
                                    torch.bmm(edge_attr[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2)])

                # With Uncertainty
                # x_j = self.lane_embed([torch.cat([torch.bmm(x_j[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2), torch.sqrt(2*(x_j[:,2:4]**2))],dim=-1), 
                #                         torch.cat([torch.bmm(edge_attr[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2), torch.sqrt(2*(edge_attr[:,2:4]**2))],dim=-1)])
