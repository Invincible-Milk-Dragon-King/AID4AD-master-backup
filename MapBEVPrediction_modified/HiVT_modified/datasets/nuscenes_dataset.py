...
def process_nuscenes(split, raw_path, centerline_used):
    ...
    # predicted map loading for baseline
    ...
    # maptr_gt_map loading for baseline
    (lane_vectors, lane_actor_index, lane_actor_vectors) = get_lane_features_maptr_gt_map(
        np.arange(0, all_node_pos.shape[0]),
        node_positions_19, 
        origin, 
        rotate_mat, 
...
def get_lane_features_maptr_gt_map(node_inds,
                      node_positions, 
                      origin: torch.Tensor,
                      rotate_mat: torch.Tensor,
                      predicted_map,
    ...
    boundary = np.array(gt_map['boundary'])

    divider = np.array(gt_map['divider'])

    if boundary.shape[0] == 0:
        centerlines = divider
        centerline_betas = predicted_map['divider_betas']
    else:
        centerlines = boundary
        centerline_betas = predicted_map['boundary_betas']
